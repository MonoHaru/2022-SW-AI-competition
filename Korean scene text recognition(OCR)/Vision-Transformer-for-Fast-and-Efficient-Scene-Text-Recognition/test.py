...
def forpred(model, criterion, evaluation_loader, converter, opt):
    ...
    # return valid_loss_avg.val(), accuracy, norm_ED, preds_str, confidence_score_list, labels, infer_time, length_of_data
    return valid_loss_avg.val(), accuracy, norm_ED, preds_strs, confidence_score_list, labels, infer_time, length_of_data
    ...
    with torch.no_grad():
        if opt.benchmark_all_eval:  # evaluation with 10 benchmark evaluation datasets
            ...
            for test_predict in test_predicts:
                extended_test_predicts.append(test_predict)
