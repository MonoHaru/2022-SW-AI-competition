...
class my_Flatten(nn.Module):
    def forward(self, x):
        sizes = x.shape
        return x.view(sizes[0], -1)

class AlexNet_FeatureExtractor(nn.Module):
    def __init__(self, input_channel, output_channel=512):
        super(AlexNet_FeatureExtractor, self).__init__()
        self.output_channel = output_channel

        self.conv1 = nn.Conv2d(in_channels=input_channel, out_channels=96, kernel_size=3, stride=4)
        self.conv2 = nn.Conv2d(in_channels=96, out_channels=256, kernel_size=5, stride=1, padding=2)
        self.conv3 = nn.Conv2d(in_channels=256, out_channels=384, kernel_size=3, stride=1, padding=1)
        self.conv4 = nn.Conv2d(in_channels=384, out_channels=384, kernel_size=3, stride=1, padding=1)
        self.conv5 = nn.Conv2d(in_channels=384, out_channels=512, kernel_size=3, stride=1, padding=1)
...
class U_Net(nn.Module):
    def __init__(self, img_ch=3, output_ch=1):
        ...
        # self.fc1 = nn.Linear(224 * 224, 1)

        # self.vitstr = create_vitstr(num_tokens=11186, model='vitstr_base_patch16_224')
