...
def forpred(model, criterion, evaluation_loader, converter, opt):
    """ for pred """
    n_correct = 0
    norm_ED = 0
    length_of_data = 0
    ...
    for i, (image_tensors, labels) in enumerate(evaluation_loader):
        ...
        if 'CTC' in opt.Prediction:
            ...
            if opt.baiduCTC:
                ...
                _, preds_index = preds.max(2)
            preds_str.append(converter.decode(preds_index.data, preds_size.data))
            ...
            # select max probabilty (greedy decoding) then decode index to character
            _, preds_index = preds.max(2)
            preds_str.append(converter.decode(preds_index, length_for_pred))
            labels = converter.decode(text_for_loss[:, 1:], length_for_loss)
...
def test_forpred(opt):
    ...
    with torch.no_grad():
        if opt.benchmark_all_eval:  # evaluation with 10 benchmark evaluation datasets
            ...
            extended_test_predicts = []
            for test_predict in test_predicts:
                extended_test_predicts.extend(test_predict)
            print(len(extended_test_predicts))
            submit['text'] = extended_test_predicts
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch_size', type=int, default=1213, help='input batch size')
    # parser.add_argument('--batch_size', type=int, default=3669, help='input batch size')
